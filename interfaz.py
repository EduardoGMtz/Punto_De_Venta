...
import sqlite3
import pandas as pd
from PyQt6.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget, QPushButton, QHBoxLayout, QLabel, QListWidget, QSplitter, QInputDialog, QLineEdit, QMessageBox, QDialog, QTextEdit
...
class PuntoDeVenta(QMainWindow):
    ...
    def agregar_al_carrito(self, producto):
        ...
        if stock > 0:
            ...
            self.lista_carrito.addItem(f"{plu} - {nombre} - ${precio}")
            self.actualizar_total()
        else:
            QMessageBox.warning(self, "Sin stock", f"El producto {nombre} no tiene stock disponible.")
    ...
    def exportar_ventas_excel(self):
        ...
        try:
            ...
            if df.empty:
                QMessageBox.warning(self, "Sin ventas", "No hay ventas para exportar.")
                return
            
            df.to_excel("ventas.xlsx", index=False)
            QMessageBox.information(self, "Exportado", "Ventas exportadas a ventas.xlsx correctamente.")
